"""
3D モック生成 Foundry Hosted Agent - エントリポイント

Foundry Hosted Agent として Azure 上にデプロイされるエージェント。
自然言語または参考画像を受け取り、AI 画像生成 → 3D モデル変換を行う。

環境変数（agent.yaml / azd env で管理）:
    FOUNDRY_PROJECT_ENDPOINT      : Azure AI Foundry プロジェクトエンドポイント
    AZURE_AI_MODEL_DEPLOYMENT_NAME: 使用するモデルのデプロイ名
...
def generate_image(
    prompt: Annotated[
        str,
        "生成したいモック画像の説明（自然言語）。"
        "TRELLIS が 3D 変換しやすいよう「白背景・正面向き・シンプルな構図」を意識する。",
    ],
...
def generate_3d_model(
    image_url: Annotated[
        str,
        "3D 変換する画像の URL（http/https）。generate_image の imageUrl を渡す。",
    ],
    quality: Annotated[
        str,
...
async def main() -> None:
    ...
    if not project_endpoint or not deployment:
        raise RuntimeError(
            "FOUNDRY_PROJECT_ENDPOINT と AZURE_AI_MODEL_DEPLOYMENT_NAME を設定してください。"
        )
