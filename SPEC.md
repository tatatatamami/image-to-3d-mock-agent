...
### 3.1 ImageTo3DMockAgent.Functions（.NET Azure Functions v4）— **未実装**
...
| `AZURE_STORAGE_CONNECTION_STRING` | 画像保存先 Blob Storage 接続文字列 | `UseDevelopmentStorage=true` |
| `AZURE_STORAGE_CONTAINER_NAME` | 画像保存先コンテナ名 | `images` |
...
| コンポーネント | 役割 | 実装状況 |
|---|---|---|
| `ImageTo3DMockAgent.Functions` | Step 2: AI 画像生成 | **未実装**（空プロジェクト） |
...
| Foundry 本体 Agent | ユーザー対話・ツール呼び出しオーケストレーション | **別途実装** |
...
| `agent-framework-azure-ai` | Microsoft Foundry Agent フレームワーク |
...
### ImageTo3DMockAgent.Api（モックモード）

```bash
# local.settings.json の IMAGE_TO_3D_API_ENDPOINT をコメントアウトまたは削除
...
- `ImageTo3DMockAgent.Functions`（画像生成）は **未実装**のため、フルフロー実現には Azure OpenAI 連携コードの追加が必要。
